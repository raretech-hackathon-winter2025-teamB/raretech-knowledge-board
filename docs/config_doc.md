...
### 12.1 目的
- 画面/部品の責務を短時間で把握できる状態を維持する。
- htmx/_hyperscript の差し替え境界を明確化する。
- 仕様変更時の影響範囲確認コストを下げる。

### 12.2 記法
- Djangoテンプレートコメントを使用する: `{# ... #}`
- JSコメントは `// ...`、CSSコメントは `/* ... */` を使用する。
- 形式は必ず `【分類】説明` に統一する。
- 句点は原則省略し、1コメント1責務で記述する。

### 12.3 分類ラベル
- `【ファイル責務】` : ファイル先頭で画面/部品の役割を示す。
- `【セクション】` : 画面内の大きな構造単位を示す。
- `【共通部品】` : includeされる再利用パーツの役割を示す。
- `【遷移】` : htmx遷移や画面切替の意図を示す。
- `【モーダル】` : モーダル領域の責務を示す。
- `【補足】` : 実装上の前提や依存を示す（必要時のみ）。

### 12.4 配置ルール
- ファイル先頭に `【ファイル責務】` を必ず1つ配置する。
- 主要ブロック直前に `【セクション】` を配置する。
- include直前には必要な場合のみ `【共通部品】` を配置する。
- 属性値内（`hx-*`, `_=`）にはコメントを書かない。
- JS/CSSでも同様に、先頭に `【ファイル責務】`、主要処理前に `【セクション】` を配置する。

### 12.5 禁止事項
- 実装と一致しない将来予定の記述をしない。
- 冗長説明（HTMLを読めば明白な説明）を避ける。
- 同一ファイル内で文体/記法を混在させない。

### 12.6 例
- `【ファイル責務】公開トップページのメイン導線と機能紹介を表示`
- `【セクション】ヒーローと認証CTA`
- `【遷移】新規登録画面へhtmxでmain領域を差し替え`
